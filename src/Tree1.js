...
function Tree1() {
    ...
    const handleCheck = (event, nodeId) => {
        ...
        if (node) {
            toggleDescendants(node, !isChecked);
        }
    ...
    const toggleDescendants = (node, isChecked) => {
        setChecked((prevChecked) =>
            isChecked
                ? [...prevChecked, String(node.id)]
                : prevChecked.filter((id) => id !== String(node.id))
        );

        if (node.children && node.children.length > 0) {
            for (const childNode of node.children) {
                toggleDescendants(childNode, isChecked);
            }
        }
    };
    useEffect(() => {
    ...
    const calculateNodeState = (node, checkedNodes) => {
        const allChildrenChecked = node.children.every((child) =>
            checkedNodes.includes(String(child.id))
        );
    ...
    const handleToggle = (event, nodeId) => {
        setExpanded((prevExpanded) =>
            prevExpanded.includes(nodeId)
                ? prevExpanded.filter((id) => id.parentId === nodeId)
                : [...prevExpanded, nodeId]
        );
    };
        ...
        return nodes.map((node) => (
            <TreeItem
                ...
                label={<FormControlLabel
                    control={
                        <Checkbox
                            checked={checked.includes(String(node.id))}
                            onChange={(event) => handleCheck(event, String(node.id))}
                            /*indeterminate={
                                !nodes[].every(p => props.products.map(i => i.label).includes(p)) &&
                                allLevelsProductLabelsMap[`${genre}-${seriesName}-${developmentCode}`].some(p => props.products.map(i => i.label).includes(p))
                            }*/
                            onClick={(event) => {
                                event.stopPropagation();
